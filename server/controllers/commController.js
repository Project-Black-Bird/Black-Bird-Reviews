module.exports = {
  createComment: (req, res) => {
    ...
    if (user_id == null) {
      res.status(400).send("Missing user_id in request body");
      return;
    }
    if (post_id == null) {
      res.status(400).send("Missing post_id in request body");
      return;
    }
    console.log("server create comment ", user_id, post_id, comment);
    db = req.app.get("db");
